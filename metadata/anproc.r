...
spl.wl <-  NULL #"1300-to-1600"#							## which wavelengths to use? leave empty c() or set to NULL for using all available wavelengths. Provide a character vector in the format e.g. c("1300-to-1600"), or c("1000-to-2000", "1300-to-1600", ...) to split along these wavelengths.
#######
dpt.pre <-  NULL##c("sgol", "msc") #						## Character vector, which of the available modules of data pre-treatments to apply. Leave at NULL for no additional data pre-treatment. For possible values see ?dpt_modules.
#######
...
#######
dpt.post <- NULL#c("sgol", "msc") #NULL#						## Character vector, which of the available modules of data pre-treatments to apply.  Leave at NULL for no additional data pre-treatment. For possible values see ?dpt_modules.
################################################################
...
pca.lo <- 1:2							## Numeric sequence of length >=2, indicating what PC components to plot in the loadingplot.
#########################
...
aqg.vars <- "C_tempLev"	#"C_variety"	 				## which variables should be use to group the data? Provide a character vector with valid class variable names to group along these variables; e.g. "C_Group", or c("C_Group", "C_FooBar", ...)
aqg.nrCorr <- TRUE #"def"						## please see the help for ?calc_aqg_args and ?plot_aqg_args for further information regarding these arguments. # en el caso de que haya diferente numero ya sea porque se eliminaron outlayers
aqg.spectra <- FALSE#"all" #   
aqg.minus <- NULL #"T1" #
aqg.mod <-  "classic" #"def" #cuando son muy unidas las lineas en el aquagrama usar classic   
aqg.TCalib <- "def"
aqg.Texp <- "def"
aqg.bootCI <- "def"
aqg.R <- "def"
